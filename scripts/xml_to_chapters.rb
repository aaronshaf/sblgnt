...
doc.xpath("//book").each do |book|
	...
	if(!File.directory?("../json/" + osisAbbreviation))
		Dir::mkdir("../json/" + osisAbbreviation)
	end
				...
				if child.text.end_with?(':1')
					if chapter != nil
						if paragraph.xpath("w").length > 0
							versesInParagraphsInChapter << versesInParagraph
							chapter.root.add_child(paragraph)
						end

						File.open("../json/#{osisAbbreviation}/#{paddedChapterNumber}-paragraphs.json","w") do |fileToWrite|
							fileToWrite.write(JSON.pretty_generate(versesInParagraphsInChapter))
						end
